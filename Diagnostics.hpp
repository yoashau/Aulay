...
std::wstring DiagnosticOperatingSystemVersion()
{
	...
	return std::to_wstring(version.dwMajorVersion) + L"." + std::to_wstring(version.dwMinorVersion) + L"." + std::to_wstring(version.dwBuildNumber);
}
...
{
	return diagnostic.logDirectory / (diagnostic.logPath.stem().wstring() + L"." + std::to_wstring(index) + L".log");
}
...
void RecordDiagnostic(std::wstring_view area, std::wstring_view message)
{
	...
	if (g_diagnosticWriter)
		g_diagnosticWriter->Push(std::move(entry));
}
...
void InitializeDiagnostics()
{
...

	{
		...
		if (!g_app.diagnostic.logDirectory.empty()) {
			g_app.diagnostic.logPath = g_app.diagnostic.logDirectory / (L"Aulay-" + std::to_wstring(GetCurrentProcessId()) + L".log");
		}
		g_app.diagnostic.privacySalt = static_cast<uint64_t>(counter.QuadPart) ^ (GetTickCount64() << 17) ^ static_cast<uint64_t>(GetCurrentProcessId());
		g_app.diagnostic.initialized = true;
	}

	auto file = std::make_shared<DiagnosticFileState>();
	file->logPath = g_app.diagnostic.logPath;
	file->logDirectory = g_app.diagnostic.logDirectory;
	...
	try {
		...
		OutputDebugStringW(L"Aulay application log worker failed to start\n");
	}
...
std::wstring BuildDiagnosticReport()
{
	...
	if (g_diagnosticWriter) {
		...
		report += L"Background log: queued=" + std::to_wstring(g_diagnosticWriter->pending.size()) + L" dropped=" + std::to_wstring(g_diagnosticWriter->totalDropped) + L" errors=" + std::to_wstring(g_diagnosticWriter->errors) + L" finished=" + std::to_wstring(g_diagnosticWriter->finished) + L"\r\n";
	}
