...
void MarkNoSound(std::wstring const& deviceId)
{
	...
	if (session == g_app.sessions.end() || session->second.phase != ConnectionPhase::Connected) return;
    ...
    std::shared_ptr<AudioFlow::State> predecessor)
{
    ...
    try {
        if(predecessor && predecessor!=flow)co_await AudioFlow::WaitFinished(predecessor);
        co_await winrt::resume_background();
        RevokeConnectionHandler(retired);
        if(retired.connection)retired.connection.Close();
    ...
    RequestDebugAudioSnapshot(L"attempt="+std::to_wstring(retired.attemptId)+L" device="+
        DiagnosticDeviceToken(deviceId)+L" reason=CLOSE_AFTER_RETURN requested-tick-ms="+std::to_wstring(GetTickCount64()));
    AudioFlow::SignalClosed(*flow);
    if(!workerStarted){flow->done=true;SetEvent(flow->finishedSignal.get());}
}
...
{
    auto session=g_app.sessions.find(deviceId);
    if(session==g_app.sessions.end() || (generation && session->second.generation!=*generation))return false;
    ...
    if(retired.cancellation)retired.cancellation->Request();
    if(retired.openedSignal)retired.openedSignal->SetEvent();
    bool cleanAudio=wasConnected && !IsStopping() && !radioWillReset &&
        !g_app.bluetooth.inProgress && !g_app.unexpectedDisconnectRecoveryPending;
    ...
    if(!flow || flow->done) {
        try {flow=AudioFlow::Start(deviceId,[id=deviceId,attempt=retired.attemptId](auto const& text){RecordConnectionDiagnostic(attempt,id,text);},true,false);}
        catch(...) {flow=std::make_shared<AudioFlow::State>();workerStarted=false;RecordConnectionDiagnostic(retired.attemptId,deviceId,L"audio-flow cleanup launch failed");}
    }
    ...
    if(markRecoveryNeeded && wasConnected && !HasConnectedSessions())g_app.bluetooth.needed=true;
    return wasConnected;
}
...
bool PrepareForcedConnection(std::wstring const& deviceId)
{
    ...
    if(it==g_app.sessions.end())return true;
    if(it->second.phase!=ConnectionPhase::Connected)return false;
	...
	int64_t connectedDuration)
{
	...
	RecordConnectionDiagnostic(
		attemptId,
		deviceId,
		L"disconnect=unexpected auto-radio-recovery=" +
			std::wstring(BluetoothRecoveryResultName(result)) +
		L" recovery-hr=" + FormatDiagnosticHresult(recoveryError) +
		L" duration-ms=" + std::to_wstring(connectedDuration));
	...
	if (result == BluetoothRecoveryResult::Success &&
        g_app.connectGenerations[deviceId] == generation &&
        g_app.reconnectEnabled && IsDesiredDevice(deviceId))
    {
        g_app.pendingAutoReconnects.insert(deviceId);
        auto found = g_discoveredDevices.find(deviceId);
        if (found != g_discoveredDevices.end()) MaybeQueuePendingReconnect(found->second);
    }
	ScheduleDeviceReconciliation();
	RefreshEmptyState();
}
...
void HandleConnectionClosed(std::wstring deviceId, uint64_t generation, AudioPlaybackConnection sender)
{
		...
		(void)g_uiDispatcher.RunAsync(
			...
			[deviceId = std::move(deviceId), generation, sender]() {
				...
				if (session == g_app.sessions.end() ||
					...
					session->second.connection != sender)
				{
					return;
				}
				...
				if (phase != ConnectionPhase::Connected)
				{
					RecordConnectionDiagnostic(
						attemptId,
						deviceId,
						L"closed observed during phase=" + std::wstring(ConnectionPhaseName(phase)) +
						L"; connection attempt retains cleanup ownership");
					return;
				}
				...
				if (g_app.unexpectedDisconnectRecoveryPending || g_app.bluetooth.inProgress)
				{
					...
					RecordConnectionDiagnostic(
						attemptId,
						deviceId,
						L"disconnect=recovery-induced auto-radio-recovery=already-running duration-ms=" +
							std::to_wstring(connectedDuration));
					return;
				}
    ...
    g_app.connectionQueueChanged.SetEvent();
				...
				RecordConnectionDiagnostic(
					attemptId,
					deviceId,
					L"disconnect=unexpected auto-radio-recovery=started active-sessions-closed=" +
						std::to_wstring(sessionIds.size()) +
					L" duration-ms=" + std::to_wstring(connectedDuration));
...
winrt::Windows::Foundation::IAsyncOperation<int32_t> ConnectDeviceAsync(ConnectionRequest request)
{
	...
	if (deviceId.empty() || IsStopping() ||
		g_app.connectGenerations[deviceId] != request.generation ||
		g_app.sessions.count(deviceId) != 0)
	{
	...
	RecordConnectionDiagnostic(
		request.attemptId,
		deviceId,
		L"attempt-start mode=" + std::wstring(ConnectionRequestModeName(request.mode)) +
		L" try=" + std::to_wstring(request.retryCount + 1) +
		L" recovery-strategy=" + std::wstring(ConnectionRecoveryStrategyName(request.recoveryStrategy)));
    ...
    if (previousTask) g_audioFlowTasks.erase(deviceId);
	...
	RecordConnectionDiagnostic(
		request.attemptId,
		deviceId,
		L"recovery-result=" + std::wstring(BluetoothRecoveryResultName(recoveryResult)) +
	...
	if (recoveryResult != BluetoothRecoveryResult::Success && recoveryResult != BluetoothRecoveryResult::NotRequired)
	{
		...
		RecordConnectionDiagnostic(
			request.attemptId,
			deviceId,
			L"attempt-complete result=bluetooth-recovery-failed recovery-result=" +
			std::wstring(BluetoothRecoveryResultName(recoveryResult)) +
			L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(attemptStarted)));
	...
	if (!device || recoveryResult == BluetoothRecoveryResult::Success ||
		request.recoveryStrategy == ConnectionRecoveryStrategy::RestartBluetooth)
	{
        ...
        for (uint32_t lookupTry = 1; !refreshedDevice && std::chrono::steady_clock::now()<lookupDeadline; ++lookupTry)
		{
			...
			catch (winrt::hresult_error const& error)
			{
				if (error.code() == HRESULT_FROM_WIN32(ERROR_TIMEOUT) ||
					error.code() == HRESULT_FROM_WIN32(ERROR_CANCELLED) || error.code() == E_ABORT)
					throw;
				RecordConnectionDiagnostic(
					request.attemptId,
					deviceId,
					L"device re-resolution try=" + std::to_wstring(lookupTry) +
					L" exception hr=" + FormatDiagnosticHresult(static_cast<HRESULT>(error.code())));
				LOG_CAUGHT_EXCEPTION();
			}
			catch (...)
			{
				...
				RecordConnectionDiagnostic(
					...
					deviceId,
					L"device re-resolution try=" + std::to_wstring(lookupTry) +
					L" exception hr=" + FormatDiagnosticHresult(error));
				LOG_CAUGHT_EXCEPTION();
			}
		...
		RecordConnectionDiagnostic(
			request.attemptId,
			deviceId,
			L"device re-resolution found=" + std::wstring(refreshedDevice ? L"true" : L"false") +
			L" retained-previous-snapshot=" + (device && !refreshedDevice ? L"true" : L"false") +
			L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(lookupStarted)));
	}
	...
	try
	{
		...
		RecordConnectionDiagnostic(
			request.attemptId,
			deviceId,
			L"try-create created=" + std::wstring(currentConnection ? L"true" : L"false") +
			L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(createStarted)));
		...
		else
		{
			...
			RecordConnectionDiagnostic(
				request.attemptId,
				deviceId,
				L"StartAsync completed state=" + std::wstring(AudioConnectionStateName(currentConnection.State())) +
				L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(startStarted)));
			...
			RecordConnectionDiagnostic(
				request.attemptId,
				deviceId,
				L"OpenAsync completed status=" + std::wstring(AudioOpenStatusName(openStatus)) +
				L" extended-error=" + FormatDiagnosticHresult(extendedError) +
				L" state=" + std::wstring(AudioConnectionStateName(stateAfterOpen)) +
				L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(openStarted)));
			if (!IsCurrentConnectionSession(deviceId, request.generation, currentConnection) || IsStopping())
			{
				auto cancelled = IsStopping() || g_app.connectGenerations[deviceId] != request.generation;
				CloseConnectionSession(deviceId, request.generation, false);
			...
			case AudioPlaybackConnectionOpenResultStatus::Success:
			{
				...
				RecordConnectionDiagnostic(
					request.attemptId,
					deviceId,
					L"opened-state wait started initial-state=" +
					std::wstring(AudioConnectionStateName(observedState)) +
					L" timeout-ms=" + std::to_wstring(CONNECTION_OPENED_WAIT_MS));
				...
				RecordConnectionDiagnostic(
					request.attemptId,
					deviceId,
					L"opened-state wait completed state=" +
					std::wstring(AudioConnectionStateName(observedState)) +
					L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(openedWaitStarted)));
	...
	if (result == ConnectionAttemptResult::Success)
	{
        ...
        try {
            auto id = deviceId; auto attempt = request.attemptId;
        ...
        } catch (...) { RecordConnectionDiagnostic(request.attemptId, deviceId, L"audio-flow monitor launch failed"); }
		...
		RecordConnectionDiagnostic(
			request.attemptId,
			deviceId,
			L"attempt-complete result=success try=" + std::to_wstring(request.retryCount + 1) +
			L" duration-ms=" + std::to_wstring(ElapsedMilliseconds(attemptStarted)));
	...
	RecordConnectionDiagnostic(
		request.attemptId,
		deviceId,
		L"attempt-complete result=" + std::wstring(ConnectionAttemptResultName(result)) +
		L" try=" + std::to_wstring(request.retryCount + 1) +
		L" recovery-needed=true duration-ms=" + std::to_wstring(ElapsedMilliseconds(attemptStarted)));
...
bool IsConnectionQueued(std::wstring const& deviceId)
{
	return std::find_if(g_app.connectionQueue.begin(), g_app.connectionQueue.end(),
		[&deviceId](const ConnectionRequest& request) { return request.deviceId == deviceId; }) != g_app.connectionQueue.end();
}
...
{
    if(deviceId.empty() || IsStopping())return;
    ...
    if((g_app.unexpectedDisconnectRecoveryPending || g_app.bluetooth.inProgress) &&
        mode==ConnectionRequestMode::Automatic)return;
    if(mode==ConnectionRequestMode::Forced && !PrepareForcedConnection(deviceId))return;
    if(g_app.sessions.count(deviceId)!=0)return;
	...
	RecordConnectionDiagnostic(
		attemptId,
		deviceId,
		L"request-queued mode=" + std::wstring(ConnectionRequestModeName(mode)) +
		L" try=" + std::to_wstring(retryCount + 1) +
		L" recovery-strategy=" + std::wstring(ConnectionRecoveryStrategyName(resolvedRecoveryStrategy)));
...
winrt::fire_and_forget ProcessConnectionQueue()
{
	try
	{
		...
		while (!g_app.connectionQueue.empty() && !IsStopping())
		{
			...
			if (IsRetryableConnectionResult(result) &&
				!IsStopping() && g_app.connectGenerations[request.deviceId] == request.generation &&
				!IsConnectionQueued(request.deviceId) && g_app.sessions.count(request.deviceId) == 0)
			{
				auto nextStrategy = SelectRetryRecoveryStrategy(request, result);
				auto retryNotBefore = std::chrono::steady_clock::now() +
					std::chrono::milliseconds(aulay::timing::RetryDelayMs(request.retryCount));
				RecordConnectionDiagnostic(
					request.attemptId,
					request.deviceId,
					L"scheduling retry strategy=" + std::wstring(ConnectionRecoveryStrategyName(nextStrategy)) +
					L" delay-ms=" + std::to_wstring(aulay::timing::RetryDelayMs(request.retryCount)) +
					L" after result=" + std::wstring(ConnectionAttemptResultName(result)));
			...
			else if (result != ConnectionAttemptResult::Success && result != ConnectionAttemptResult::Cancelled)
			{
				RecordConnectionDiagnostic(
					request.attemptId,
					request.deviceId,
					L"no further retry result=" + std::wstring(ConnectionAttemptResultName(result)) +
					L" retries-used=" + std::to_wstring(request.retryCount));
			}
		}
...
void MaybeQueuePendingReconnect(DeviceInformation const& device)
{
	...
	if (g_app.pendingAutoReconnects.count(deviceId) == 0 ||
		g_app.sessions.count(deviceId) != 0 || IsConnectionQueued(deviceId))
	{
		return;
	}
