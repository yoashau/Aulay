...
int main()
{
	...
	q->Start([&](auto const& batch) {
        ...
        for(auto const& s:batch)lines.push_back(s); }, [&] { closed = true; });
	assert(q->Push(L"first"));
	...
	for (size_t i = 1; i < q->maxEntries; ++i)
		assert(q->Push(L"queued"));
	assert(!q->Push(L"overflow"));
	q->Stop();
	assert(!q->Push(L"after-stop"));
	release.set_value();
	assert(q->WaitFor(2s));
	assert(closed && lines.front() == L"first" && lines[1] == L"second");
	assert(lines.back().find(L"app-log-queue-dropped=1") != std::wstring::npos);
	auto failed = std::make_shared<aulay::logging::Queue>();
	bool finally = false;
	failed->Start([](auto const&) { throw 1; }, [&] { finally = true; });
	failed->Push(L"failure");
	assert(failed->WaitFor(2s));
	assert(finally && failed->errors == 1);
