...
std::wstring DebugAudioToken(std::wstring_view value, uint64_t salt)
{
	...
	for (auto c : value) { hash ^= static_cast<uint16_t>(c); hash *= 1099511628211ull; }
	wchar_t text[20]{};
...
std::vector<std::wstring> CollectDebugAudioSnapshot(uint64_t salt)
{
	...
	lines.push_back(L"capture-start tick-ms=" + std::to_wstring(started) +
		L" scope=system-render-observation audio-content=not-recorded");
	...
	try
	{
		...
		for (auto role : { eConsole, eMultimedia, eCommunications })
		{
			...
			lines.push_back(L"default-output role=" + std::to_wstring(static_cast<int>(role)) +
				L" hr=" + DebugAudioHresult(hr) +
				(SUCCEEDED(hr) ? L" endpoint=" + DebugEndpointToken(endpoint.get(), salt) : L""));
		}
		...
		for (UINT i = 0; i < count && i < 16; ++i)
		{
			try
			{
				...
				lines.push_back(prefix + L" volume-hr=" + DebugAudioHresult(hr) + (SUCCEEDED(hr)
					? L" volume=" + DebugAudioValue<float>([&](auto p) { return volume->GetMasterVolumeLevelScalar(p); }) +
					  L" mute=" + DebugAudioValue<BOOL>([&](auto p) { return volume->GetMute(p); }) : L""));
				...
				lines.push_back(prefix + L" meter-hr=" + DebugAudioHresult(hr) + (SUCCEEDED(hr)
					? L" peak=" + DebugAudioValue<float>([&](auto p) { return meter->GetPeakValue(p); }) : L""));
				...
				if (FAILED(hr)) { lines.push_back(prefix + L" sessions-hr=" + DebugAudioHresult(hr)); continue; }
				...
				for (int j = 0; j < sessionCount && j < 64; ++j)
				{
					...
					if (FAILED(hr)) { lines.push_back(prefix + L" session-read-hr=" + DebugAudioHresult(hr)); continue; }
					auto entry = prefix + L" session-index=" + std::to_wstring(j) + L" state=" +
						DebugAudioValue<AudioSessionState>([&](auto p) { return session->GetState(p); });
					...
					if (auto simple = session.try_as<ISimpleAudioVolume>())
						entry += L" volume=" + DebugAudioValue<float>([&](auto p) { return simple->GetMasterVolume(p); }) +
							L" mute=" + DebugAudioValue<BOOL>([&](auto p) { return simple->GetMute(p); });
			...
			catch (...) { lines.push_back(L"endpoint-read-error hr=" + DebugAudioHresult(winrt::to_hresult())); }
		}
	}
	catch (...) { lines.push_back(L"capture-error hr=" + DebugAudioHresult(winrt::to_hresult())); }
	lines.push_back(L"capture-end duration-ms=" + std::to_wstring(GetTickCount64() - started) +
		L" interpretation=instantaneous-observation-not-proof-of-audibility;session-enumeration-may-be-incomplete");
	return lines;
}
