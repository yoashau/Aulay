...
const wchar_t* BluetoothStageName(BluetoothRecoveryStage stage)
{
	switch (stage)
	{
	case BluetoothRecoveryStage::Idle: return L"idle";
	case BluetoothRecoveryStage::RequestingAccess: return L"requesting-access";
	case BluetoothRecoveryStage::FindingRadio: return L"finding-radio";
	case BluetoothRecoveryStage::TurningOff: return L"turning-off";
	case BluetoothRecoveryStage::WaitingForOff: return L"waiting-for-off";
	case BluetoothRecoveryStage::TurningOn: return L"turning-on";
	case BluetoothRecoveryStage::WaitingForOn: return L"waiting-for-on";
	case BluetoothRecoveryStage::Succeeded: return L"succeeded";
	case BluetoothRecoveryStage::Failed: return L"failed";
...
const wchar_t* BluetoothRecoveryResultName(BluetoothRecoveryResult result)
{
	switch (result)
	{
	case BluetoothRecoveryResult::NotRequired: return L"not-required";
	case BluetoothRecoveryResult::Success: return L"success";
	case BluetoothRecoveryResult::AccessDenied: return L"access-denied";
	case BluetoothRecoveryResult::RadioUnavailable: return L"radio-unavailable";
	case BluetoothRecoveryResult::TurnOffDenied: return L"turn-off-denied";
	case BluetoothRecoveryResult::TurnOffTimedOut: return L"turn-off-timed-out";
	case BluetoothRecoveryResult::TurnOnDenied: return L"turn-on-denied";
	case BluetoothRecoveryResult::TurnOnTimedOut: return L"turn-on-timed-out";
	case BluetoothRecoveryResult::UnexpectedFailure: return L"unexpected-failure";
	case BluetoothRecoveryResult::OperationTimedOut: return L"operation-timed-out";
	case BluetoothRecoveryResult::Cancelled: return L"cancelled";
	default: return L"unknown";
	}
}
...
const wchar_t* RadioAccessStatusName(winrt::Windows::Devices::Radios::RadioAccessStatus status)
{
	...
	switch (status)
	{
	case RadioAccessStatus::Unspecified: return L"unspecified";
	case RadioAccessStatus::Allowed: return L"allowed";
	case RadioAccessStatus::DeniedByUser: return L"denied-by-user";
	case RadioAccessStatus::DeniedBySystem: return L"denied-by-system";
	default: return L"unknown";
	}
}
...
const wchar_t* ConnectionAttemptResultName(ConnectionAttemptResult result)
{
	switch (result)
	{
	case ConnectionAttemptResult::Cancelled: return L"cancelled";
	case ConnectionAttemptResult::Success: return L"success";
	case ConnectionAttemptResult::BluetoothRecoveryFailed: return L"bluetooth-recovery-failed";
	case ConnectionAttemptResult::DeviceUnavailable: return L"device-unavailable";
	case ConnectionAttemptResult::StartFailed: return L"start-failed";
	case ConnectionAttemptResult::OpenTimedOut: return L"open-timed-out";
	case ConnectionAttemptResult::DeniedBySystem: return L"denied-by-system";
	case ConnectionAttemptResult::OpenFailed: return L"open-failed";
	case ConnectionAttemptResult::ClosedUnexpectedly: return L"closed-unexpectedly";
	case ConnectionAttemptResult::UnexpectedFailure: return L"unexpected-failure";
	case ConnectionAttemptResult::DeadlineExceeded: return L"deadline-exceeded";
	default: return L"unknown";
	}
}
...
const wchar_t* ConnectionRequestModeName(ConnectionRequestMode mode)
{
	switch (mode)
	{
	case ConnectionRequestMode::Automatic: return L"automatic";
	case ConnectionRequestMode::Quick: return L"quick";
	case ConnectionRequestMode::Forced: return L"forced";
	default: return L"unknown";
	}
}
...
const wchar_t* ConnectionRecoveryStrategyName(ConnectionRecoveryStrategy strategy)
{
	switch (strategy)
	{
	case ConnectionRecoveryStrategy::Direct: return L"direct";
	case ConnectionRecoveryStrategy::RestartBluetooth: return L"restart-bluetooth";
	default: return L"unknown";
	}
}
...
const wchar_t* ConnectionPhaseName(ConnectionPhase phase)
{
	switch (phase)
	{
	case ConnectionPhase::RecoveringBluetooth: return L"recovering-bluetooth";
	case ConnectionPhase::Starting: return L"starting";
	case ConnectionPhase::Opening: return L"opening";
...
const wchar_t* DeviceWatcherStatusName(DeviceWatcherStatus status)
{
	switch (status)
	{
	case DeviceWatcherStatus::Created: return L"created";
	case DeviceWatcherStatus::Started: return L"started";
	case DeviceWatcherStatus::EnumerationCompleted: return L"enumeration-completed";
	case DeviceWatcherStatus::Stopping: return L"stopping";
	case DeviceWatcherStatus::Stopped: return L"stopped";
	case DeviceWatcherStatus::Aborted: return L"aborted";
	default: return L"unknown";
	}
}
	...
	uint64_t marker, bool captureAudio)
{
	...
	if (session == g_app.sessions.end()) return;
	auto const& current = session->second;
	std::wstring state = L"unavailable";
	try { if (current.connection) state = AudioConnectionStateName(current.connection.State()); }
	catch (...) { state += L":" + FormatDiagnosticHresult(static_cast<HRESULT>(winrt::to_hresult())); }
	auto detail = std::wstring(reason) + L" marker=" + std::to_wstring(marker) +
		L" generation=" + std::to_wstring(current.generation) +
		L" mode=" + ConnectionRequestModeName(current.requestMode) +
		L" phase=" + ConnectionPhaseName(current.phase) + L" state=" + state +
		L" reports=" + std::to_wstring(current.noSoundReports) +
		L" connected-ms=" + std::to_wstring(current.phase == ConnectionPhase::Connected
			? ElapsedMilliseconds(current.connectedAt) : 0) +
		L" radio-stage=" + BluetoothStageName(g_app.bluetooth.stage) +
		L" radio-result=" + BluetoothRecoveryResultName(g_app.bluetooth.lastResult) +
		L" audio-monitor={" + DebugAudioMonitorStatus() + L"}";
	auto previous = g_debugLastDisconnect.find(deviceId);
	if (previous != g_debugLastDisconnect.end())
	{
		detail += L" previous-disconnect-attempt=" + std::to_wstring(previous->second.attemptId) +
			L" previous-disconnect-generation=" + std::to_wstring(previous->second.generation) +
			L" since-disconnect-ms=" + std::to_wstring(GetTickCount64() - previous->second.tick);
	}
	else detail += L" previous-disconnect=none-in-this-run";
	RecordConnectionDiagnostic(current.attemptId, deviceId, detail);
	if (captureAudio)
		RequestDebugAudioSnapshot(L"attempt=" + std::to_wstring(current.attemptId) +
			L" device=" + DiagnosticDeviceToken(deviceId) + L" marker=" + std::to_wstring(marker) +
			L" reason=" + std::wstring(reason) + L" requested-tick-ms=" + std::to_wstring(GetTickCount64()));
}
...
{
	return !g_app.sessions.empty() || !g_app.connectionQueue.empty() ||
		g_app.connectionWorkerRunning || g_app.unexpectedDisconnectRecoveryPending;
}
