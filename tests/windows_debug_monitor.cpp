...
{
	float value=0.5f;HRESULT hr=S_OK;
	HRESULT __stdcall GetPeakValue(float* out) noexcept override {*out=value;return hr;}
	HRESULT __stdcall GetMeteringChannelCount(UINT* out) noexcept override {*out=1;return S_OK;}
	HRESULT __stdcall GetChannelsPeakValues(UINT,float* out) noexcept override {*out=value;return hr;}
	HRESULT __stdcall QueryHardwareSupport(DWORD* out) noexcept override {*out=0;return S_OK;}
};
...
int main(int argc,char** argv)
{
	...
	try
	{
		auto s=std::make_shared<DebugAudio::State>();s->directory=dir;s->run=L"fixture";s->salt=42;
		for(int i=0;i<4097;++i)s->Push({GetTickCount64(),L"load"});
		...
		s->queue.clear();s->pendingSnapshots=0;
		s->mutex.lock();
		std::thread callback([&]{s->Push({GetTickCount64(),L"callback"},true);});callback.join();s->mutex.unlock();
		check(s->dropped==2&&s->queue.empty(),"callback does not block on queue lock; loss counted");
		{
			...
			journal.Write(GetTickCount64(),L"POST_MARKER_PLAYING_SAMPLE");journal.Flush();
			...
			journal.bundles.front().until=0;journal.Write(GetTickCount64(),L"after-window");
			...
			journal.bundles.back().bytes=16*1024*1024;journal.Write(GetTickCount64(),L"limit");
			...
			journal.bytes=32*1024*1024;journal.Write(GetTickCount64(),L"ROTATED");journal.Flush();
			...
			journal.Mark(GetTickCount64(),L"USER_NO_SOUND marker=3");journal.Finish();
		...
		{
			DebugAudio::State history;history.directory=dir;history.run=L"history";
			DebugAudio::Journal journal(history);
			journal.history.emplace_back(GetTickCount64()-120001,"EXPIRED");journal.historyBytes=7;
			...
			journal.history.emplace_back(GetTickCount64(),std::string(8*1024*1024,'x'));journal.historyBytes+=8*1024*1024;
			...
			for(int i=0;i<21;++i)journal.Mark(GetTickCount64(),L"USER_NO_SOUND retention-test");
		...
		{
			DebugAudio::State bad;bad.directory=dir/L"missing-directory";bad.run=L"bad";
			DebugAudio::Journal journal(bad);journal.Write(GetTickCount64(),L"write failure");journal.Mark(GetTickCount64(),L"marker failure");journal.Flush();
		...
		}
		{
			...
			auto dev=winrt::make_self<DebugAudio::DeviceEvents>(s);dev->OnDefaultDeviceChanged(eRender,eMultimedia,L"fixture-device");
			...
			AUDIO_VOLUME_NOTIFICATION_DATA data{};data.fMasterVolume=0.4f;data.bMuted=TRUE;data.nChannels=1;data.afChannelVolumes[0]=0.3f;vol->OnNotify(&data);
			check(s->queue.back().text.find(L"ch0=0.300000")!=std::wstring::npos,"per-channel endpoint volume callback");
		}
		s->stopping=true;auto count=s->queue.size();s->Push({GetTickCount64(),L"after-stop",true,true});
		check(s->queue.size()==count&&s->pendingSnapshots==0,"stop rejects late work");
		DebugAudio::Peak peak;peak.Read(nullptr);auto missing=peak.Take();
		...
		peak.Read(fake.get());fake->value=0;peak.Read(fake.get());fake->value=1e-9f;peak.Read(fake.get());
		...
		peak.Read(fake.get());check(peak.Take().find(L"peak-max=0 ")==std::wstring::npos,"tiny positive peaks not rounded to zero");
		...
		if(integration)
		{
			...
			winrt::com_ptr<IMMDevice> endpoint;winrt::check_hresult(enumerator->GetDefaultAudioEndpoint(eRender,eMultimedia,endpoint.put()));
			winrt::com_ptr<IAudioClient> client;winrt::check_hresult(endpoint->Activate(__uuidof(IAudioClient),CLSCTX_ALL,nullptr,client.put_void()));
			WAVEFORMATEX* raw=nullptr;winrt::check_hresult(client->GetMixFormat(&raw));wil::unique_cotaskmem_ptr<WAVEFORMATEX> format(raw);
			GUID sessionId{};winrt::check_hresult(CoCreateGuid(&sessionId));
			...
			winrt::com_ptr<ISimpleAudioVolume> volume;winrt::check_hresult(client->GetService(__uuidof(ISimpleAudioVolume),volume.put_void()));
			// Muting only this test session: no change to existing sessions/device.
			winrt::check_hresult(volume->SetMute(TRUE,nullptr));
			UINT32 size=0;winrt::check_hresult(client->GetBufferSize(&size));
			BYTE* buffer=nullptr;winrt::check_hresult(render->GetBuffer(size,&buffer));winrt::check_hresult(render->ReleaseBuffer(size,AUDCLNT_BUFFERFLAGS_SILENT));
			EnqueueDebugAudioEvent(L"TEST_USER_PLAY after-open=true own-silent-session=true");
			winrt::check_hresult(client->Start());
			for(int i=0;i<250;++i)
			{
				UINT32 used=0;winrt::check_hresult(client->GetCurrentPadding(&used));
				if(size>used){winrt::check_hresult(render->GetBuffer(size-used,&buffer));winrt::check_hresult(render->ReleaseBuffer(size-used,AUDCLNT_BUFFERFLAGS_SILENT));}
				if(i==75)EnqueueDebugAudioEvent(L"attempt=80 marker=1 reason=USER_NO_SOUND",true,true);
				if(i==125)winrt::check_hresult(volume->SetMasterVolume(0.4f,nullptr));
				std::this_thread::sleep_for(std::chrono::milliseconds(10));
			}
			winrt::check_hresult(client->Stop());EnqueueDebugAudioEvent(L"TEST_USER_STOP");
			std::this_thread::sleep_for(std::chrono::milliseconds(1500));
		}
		else EnqueueDebugAudioEvent(L"attempt=80 marker=1 reason=USER_NO_SOUND",true,true);
		EnqueueDebugAudioEvent(L"attempt=80 reason=CLOSE_AFTER_RETURN",true);
		StopDebugAudioMonitor();
		for(int i=0;i<1000&&!DebugAudioMonitorFinished();++i)std::this_thread::sleep_for(std::chrono::milliseconds(10));
		...
		if(integration)cleanup.release();
	}
	catch(...){++failed;std::wcout<<L"EXCEPTION "<<DebugAudioHresult(winrt::to_hresult())<<L'\n';}
