...
enum class Decision { Wait, Healthy, Paused, Unknown, Cancelled, Recover, AlreadyAttempted };
...
public:
    ...
    Decision Observe(const Observation& o) noexcept {
        ...
        if (attempted_) return Decision::AlreadyAttempted;
        if (!pending_ || o.tickMs < last_ || o.tickMs - last_ > maxObservationGapMs) {
            pending_ = true;
            began_ = o.tickMs;
        }
        last_ = o.tickMs;
        if (o.tickMs - began_ < noFramesMs) return Decision::Wait;
        pending_ = false;
