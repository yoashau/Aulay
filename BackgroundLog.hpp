...
struct Queue : std::enable_shared_from_this<Queue> {
    ...
    bool Push(std::wstring entry) {
        ...
        if(pending.size()>=maxEntries || size>maxBytes-bytes){++dropped;++totalDropped;wake.notify_one();return false;}
        pending.push_back(std::move(entry));bytes+=size;wake.notify_one();return true;
    }
    void Stop() {std::lock_guard<std::mutex> lock(mutex);stopping=true;wake.notify_all();}
            ...
            try {
                for(;;) {
                    ...
                    bool last=false;
                    {
                        ...
                        while(!self->pending.empty()){batch.push_back(std::move(self->pending.front()));self->pending.pop_front();}
                        self->bytes=0;
                        if(self->dropped){batch.push_back(L"[diagnostics] app-log-queue-dropped="+std::to_wstring(self->dropped)+L"\r\n");self->dropped=0;}
                        last=self->stopping;
                    }
                    if(!batch.empty())write(batch); // deliberately outside producer mutex
                    if(last)break;
                }
            } catch(...) {std::lock_guard<std::mutex> lock(self->mutex);++self->errors;}
            try {close();} catch(...) {std::lock_guard<std::mutex> lock(self->mutex);++self->errors;}
            {std::lock_guard<std::mutex> lock(self->mutex);self->finished=true;self->stopping=true;}
