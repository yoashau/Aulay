...
HWND g_hWnd = nullptr;
struct { bool reconnectEnabled = false; std::vector<std::wstring> desiredDevices; } g_app;
...
{
	++checks;
	if (!value) { ++failures; std::cout << "FAIL " << name << '\n'; }
}
...
struct ControlledAction : winrt::implements<ControlledAction, IAsyncAction, winrt::Windows::Foundation::IAsyncInfo>
{
	...
	AsyncStatus Status() { std::lock_guard<std::mutex> lock(mutex); return status; }
	winrt::hresult ErrorCode() { std::lock_guard<std::mutex> lock(mutex); return error; }
	void Cancel() { cancelCalled.SetEvent(); } // deliberately ignores cancellation
	void Close() {}
	void GetResults() { std::lock_guard<std::mutex> lock(mutex); winrt::check_hresult(error); }
	AsyncActionCompletedHandler Completed() { std::lock_guard<std::mutex> lock(mutex); return handler; }
	void Completed(AsyncActionCompletedHandler const& value)
	{
		AsyncStatus observed;
		{ std::lock_guard<std::mutex> lock(mutex); handler = value; observed = status; }
		if (observed != AsyncStatus::Started) value(*this, observed);
	}
	void Finish(HRESULT result = S_OK)
	{
		...
		AsyncStatus observed;
		{
			...
			error = result; status = FAILED(result) ? AsyncStatus::Error : AsyncStatus::Completed;
			callback = handler; observed = status;
		}
		if (callback) callback(*this, observed);
	}
};

IAsyncOperation<int> ImmediateValue() { co_return 7; }
...
void TestDeadlines()
{
	...
	}
	{
		...
		try { AwaitBounded(action.as<IAsyncAction>(), start + milliseconds(30)).get(); }
		catch (winrt::hresult_error const& error) { timedOut = error.code() == HRESULT_FROM_WIN32(ERROR_TIMEOUT); }
	...
	}
	{
		...
		start = steady_clock::now();
		try { AwaitBounded(action.as<IAsyncAction>(), start + seconds(5), cancellation).get(); }
		catch (winrt::hresult_error const& error) { cancelled = error.code() == HRESULT_FROM_WIN32(ERROR_CANCELLED); }
		shutdown.join();
	...
	{
		auto cancellation = std::make_shared<AsyncCancellation>(); cancellation->Request();
		bool cancelled = false;
		try { AwaitBounded(ImmediateValue(), steady_clock::now() + seconds(1), cancellation).get(); }
		catch (winrt::hresult_error const& error) { cancelled = error.code() == HRESULT_FROM_WIN32(ERROR_CANCELLED); }
		Check(cancelled, "pre-cancelled operation is rejected");
	}
	{
		auto action = winrt::make_self<ControlledAction>(); action->Finish(E_ACCESSDENIED);
		bool preserved = false;
		try { AwaitBounded(action.as<IAsyncAction>(), steady_clock::now() + seconds(1)).get(); }
		catch (winrt::hresult_error const& error) { preserved = error.code() == E_ACCESSDENIED; }
	...
	}
	{
		...
		AwaitBounded(first.as<IAsyncAction>(), deadline).get(); done1.join();
		...
		bool timedOut = false;
		try { AwaitBounded(second.as<IAsyncAction>(), deadline).get(); }
		catch (winrt::hresult_error const& error) { timedOut = error.code() == HRESULT_FROM_WIN32(ERROR_TIMEOUT); }
		done2.join();
...
void TestSettings()
{
	...
	auto portable = root / L"portable"; auto fallback = root / L"fallback";
	fs::create_directories(portable);
	auto cleanup = wil::scope_exit([&] {
		SetDirectoryAccess(portable, true);
		if (fs::is_directory(fallback)) SetDirectoryAccess(fallback, true);
	...
	g_app.reconnectEnabled = true; g_app.desiredDevices = { L"old-phone" };
	SaveSettings();
	...
	{
		std::ofstream stream(g_settingsStorage.fallbackPath); stream << "{ invalid json";
	}
	LoadSettings();
...
void ProbeLocalRouting()
{
	...
	for (auto const& adapter : adapters) instances.push_back(DeviceInterfaceInstanceId(std::wstring(adapter.Id())));
	...
	for (auto const& device : devices)
	{
		...
		if (index)
		{
			if (std::any_of(ancestors.begin(), ancestors.end(), [&](auto const& id) {
				return SameDeviceInstance(id, instances[*index]); })) ++ancestorMatches;
			auto start = steady_clock::now();
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		if (argc > 1 && std::string(argv[1]) == "--probe") ProbeLocalRouting();
	}
	catch (winrt::hresult_error const& error)
	{
		++failures; std::cout << "EXCEPTION HRESULT=" << std::hex << static_cast<uint32_t>(error.code()) << std::dec << '\n';
	}
	catch (std::exception const& error) { ++failures; std::cout << "EXCEPTION " << error.what() << '\n'; }
