...
inline std::wstring ResolveEndpoint(std::wstring const& interfaceId)
{
	...
	DEVPROPTYPE type = 0;
	ULONG cb = sizeof(parentId);
	if (CM_Get_Device_Interface_PropertyW(interfaceId.c_str(), &DEVPKEY_Device_InstanceId,
			&type, reinterpret_cast<BYTE*>(parentId), &cb, 0)
			!= CR_SUCCESS
		|| type != DEVPROP_TYPE_STRING)
		return {};
	parentId[MAX_DEVICE_ID_LEN - 1] = 0;
	if (!Starts(parentId, L"BTHENUM\\{0000110A-0000-1000-8000-00805F9B34FB}"))
		return {};
	DEVINST parent = 0;
	if (CM_Locate_DevNodeW(&parent, parentId, CM_LOCATE_DEVNODE_PHANTOM) != CR_SUCCESS)
		return {};
	wchar_t service[64] {};
	cb = sizeof(service);
	if (CM_Get_DevNode_PropertyW(parent, &DEVPKEY_Device_Service, &type, reinterpret_cast<BYTE*>(service), &cb, 0) != CR_SUCCESS || type != DEVPROP_TYPE_STRING || _wcsicmp(service, L"BthA2dp") != 0)
		return {};
	constexpr auto flags = CM_GETIDLIST_FILTER_ENUMERATOR;
	ULONG chars = 0;
	if (CM_Get_Device_ID_List_SizeW(&chars, L"SWD\\MMDEVAPI", flags) != CR_SUCCESS || chars < 2 || chars > 1024 * 1024)
		return {};
	std::vector<wchar_t> ids(chars + 1, 0);
	if (CM_Get_Device_ID_ListW(L"SWD\\MMDEVAPI", ids.data(), chars, flags) != CR_SUCCESS)
		return {};
	std::wstring selected;
	for (auto id = ids.data(); *id; id += wcslen(id) + 1) {
		if (!Starts(id, L"SWD\\MMDEVAPI\\{0.0.1.00000000}."))
			continue;
		DEVINST child = 0;
		if (CM_Locate_DevNodeW(&child, id, CM_LOCATE_DEVNODE_PHANTOM) != CR_SUCCESS)
			continue;
		wchar_t actualParent[MAX_DEVICE_ID_LEN] {};
		cb = sizeof(actualParent);
		if (CM_Get_DevNode_PropertyW(child, &DEVPKEY_Device_Parent, &type,
				reinterpret_cast<BYTE*>(actualParent), &cb, 0)
				!= CR_SUCCESS
			|| type != DEVPROP_TYPE_STRING)
			continue;
		actualParent[MAX_DEVICE_ID_LEN - 1] = 0;
		if (_wcsicmp(actualParent, parentId) != 0)
			continue;
		if (!selected.empty())
			return {}; // ambiguous mapping is not permission to guess
...
{
	ComPtr<IMMDevice> d;
	auto hr = en->GetDevice(id.c_str(), &d);
...
inline HRESULT Jack(IMMDevice* d, bool& connected)
{
	...
	if (FAILED(hr))
		return hr;
	ComPtr<IConnector> from, to;
	ComPtr<IPart> part;
	ComPtr<IKsJackDescription> jack;
	if (FAILED(hr = topology->GetConnector(0, &from)) || FAILED(hr = from->GetConnectedTo(&to)) || FAILED(hr = to.As(&part)) || FAILED(hr = part->Activate(CLSCTX_INPROC_SERVER, __uuidof(IKsJackDescription), reinterpret_cast<void**>(jack.GetAddressOf()))))
		return hr;
	UINT count = 0;
	if (FAILED(hr = jack->GetJackCount(&count)))
		return hr;
	if (count != 1)
		return E_UNEXPECTED;
	KSJACK_DESCRIPTION desc {};
	hr = jack->GetJackDescription(0, &desc);
	if (SUCCEEDED(hr))
		connected = desc.IsConnected != FALSE;
	return hr;
}
...
struct EndpointEvents : winrt::implements<EndpointEvents, IMMNotificationClient> {
	...
	explicit EndpointEvents(std::wstring id)
		: endpoint(std::move(id))
	{
	}
	HRESULT STDMETHODCALLTYPE OnDeviceStateChanged(LPCWSTR id, DWORD) noexcept override
	{
		Notify(id);
		return S_OK;
	}
	HRESULT STDMETHODCALLTYPE OnDeviceAdded(LPCWSTR id) noexcept override
	{
		Notify(id);
		return S_OK;
	}
	...
	HRESULT STDMETHODCALLTYPE OnPropertyValueChanged(LPCWSTR id, const PROPERTYKEY) noexcept override
	{
		Notify(id);
		return S_OK;
	}
	void Notify(LPCWSTR id) noexcept
	{
		if (id && _wcsicmp(id, endpoint.c_str()) == 0)
...
inline bool WaitEndpointState(IMMDeviceEnumerator* en, std::wstring const& id, HANDLE changed, DWORD wanted, DWORD timeoutMs = 2000)
{
	...
	for (;;) {
		DWORD value = 0;
		if (SUCCEEDED(EndpointState(en, id, value)) && (value & wanted) != 0)
			return true;
...
inline void RequestStop(State& s) noexcept
{
	s.stop = true;
	SetEvent(s.wake.get());
}
inline void SignalClosed(State& s) noexcept
{
	s.closed = true;
	SetEvent(s.closedSignal.get());
}
inline winrt::Windows::Foundation::IAsyncAction WaitFinished(std::shared_ptr<State> s)
{
	...
	if (s->stop && !s->closed)
		co_await winrt::resume_on_signal(s->closedSignal.get());
}
inline HRESULT Cycle(State& s, IMMDeviceEnumerator* en, std::wstring const& id, bool closing)
{
	...
	if ((!closing && s.stop) || ResolveEndpoint(s.interfaceId) != id)
		return E_ABORT;
	DWORD original = 0;
	auto hr = EndpointState(en, id, original);
	if (FAILED(hr))
		return hr;
	if (original != DEVICE_STATE_ACTIVE && original != DEVICE_STATE_UNPLUGGED)
		return S_FALSE;
	...
	struct Restore {
		...
		{
			if (needed)
				pc->SetEndpointVisibility(id.c_str(), true);
			state.cycling = false;
		}
	} restore { pc.Get(), id, s };
	...
	do {
		...
		restored = SUCCEEDED(enabled) && WaitEndpointState(en, id, events->changed.get(), DEVICE_STATE_ACTIVE | DEVICE_STATE_UNPLUGGED, static_cast<DWORD>((std::min)(remaining, ULONGLONG(2000))));
		if (restored)
			break;
		auto now = GetTickCount64();
	...
	Write(s, L"audio-flow cycle disable=" + Hr(disabled) + L" enable=" + Hr(enabled) + L" disabled-observed=" + std::to_wstring(observed) + L" restored=" + std::to_wstring(restored));
	if (!restored)
		return FAILED(enabled) ? enabled : E_FAIL;
...
struct Capture {
	...
	{
		Close();
		auto hr = d->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr, reinterpret_cast<void**>(client.GetAddressOf()));
		if (FAILED(hr))
			return hr;
		WAVEFORMATEX* format = nullptr;
		hr = client->GetMixFormat(&format);
		if (SUCCEEDED(hr)) {
			hr = client->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 10000000, 0, format, nullptr);
		}
	...
	HRESULT Drain(std::uint64_t& frames)
	{
		...
		for (unsigned i = 0; i < 128; ++i) {
			UINT32 packet = 0;
			auto hr = capture->GetNextPacketSize(&packet);
			if (FAILED(hr) || !packet)
				return hr;
			BYTE* data = nullptr;
			UINT32 n = 0;
			DWORD flags = 0;
			hr = capture->GetBuffer(&data, &n, &flags, nullptr, nullptr);
			if (FAILED(hr))
				return hr;
			frames += n;
			hr = capture->ReleaseBuffer(n);
			if (FAILED(hr))
				return hr;
		}
		return S_OK;
...
inline void Run(std::shared_ptr<State> s) noexcept
{
	...
	if (FAILED(initialized)) {
		s->cleanupResult = initialized;
		Write(*s, L"audio-flow COM failed " + Hr(initialized));
		s->done = true;
		return;
	}
	try {
		...
		Capture capture;
		aulay::flow::RecoveryPolicy policy;
		std::wstring endpoint;
		ULONGLONG nextResolve = 0, nextCapture = 0, verifyUntil = 0;
		bool mappedLogged = false, errorLogged = false;
		while (!s->stop) {
			auto tick = GetTickCount64();
			if (endpoint.empty() && tick >= nextResolve) {
				endpoint = ResolveEndpoint(s->interfaceId);
				nextResolve = tick + 1000;
			}
			if (!endpoint.empty() && !mappedLogged) {
				Write(*s, L"audio-flow capture endpoint mapped from exact 110A child");
				mappedLogged = true;
			}
			ComPtr<IMMDevice> d;
			DWORD deviceState = 0;
			bool jack = false, known = false;
			std::uint64_t frames = 0;
			bool active = !endpoint.empty() && SUCCEEDED(en->GetDevice(endpoint.c_str(), &d)) && SUCCEEDED(d->GetState(&deviceState)) && deviceState == DEVICE_STATE_ACTIVE;
			if (active)
				known = SUCCEEDED(Jack(d.Get(), jack));
			bool started = false, readOk = false;
			if (active && known && jack) {
				if (!capture.client && tick >= nextCapture) {
					hr = capture.Open(d.Get());
					nextCapture = tick + 1000;
					if (FAILED(hr) && !errorLogged) {
						Write(*s, L"audio-flow capture unavailable " + Hr(hr));
						errorLogged = true;
					}
				}
				started = bool(capture.client);
				if (started) {
					hr = capture.Drain(frames);
					readOk = SUCCEEDED(hr);
					if (!readOk)
						capture.Close();
				}
			...
			if (decision == aulay::flow::Decision::Recover) {
				...
				if (result == S_OK)
					verifyUntil = GetTickCount64() + 10000;
			}
			if (verifyUntil && frames) {
				Write(*s, L"audio-flow recovery verified frames=" + std::to_wstring(frames));
				verifyUntil = 0;
			}
			if (verifyUntil && tick >= verifyUntil) {
				Write(*s, L"audio-flow recovery not verified; no radio escalation");
				verifyUntil = 0;
			}
		...
		s->cleanupResult = error;
		Write(*s, L"audio-flow worker failed " + Hr(error));
	}
	CoUninitialize();
	s->done = true;
}
